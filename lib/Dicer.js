...
function partStream(opts, read) {
  return new ReadableStream(Object.assign({
    read: read
  }, opts));
}
...
Dicer.prototype._write = function(data, encoding, cb) {
  ...
  if (this._headerFirst && this._isPreamble) {
    if (!this._part) {
      this._part = partStream(this._partOpts);
...
Dicer.prototype._oninfo = function(isMatch, data, start, end) {
  ...
  if (data && start < end && !this._ignoreData) {
    if (this._isPreamble || !this._inHeader) {
      if (buf) this._part.push(buf);
      if (!this._part.push(data.slice(start, end))) {
        this._sourcePause();
      }
